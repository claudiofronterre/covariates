...
pkgs = c("sf", "tmap", "tmaptools", "readr")
pacman::p_load(pkgs, character.only = T)

# LOAD THE DATASETS ------------------------------------------------------------
malaria <- read_csv("data/TZ_2015.csv")
...
pal <- get_brewer_pal("-RdYlBu", n = 8, contrast = c(0, 1), plot = F)
...
tm_shape(africa, filter = africa$COUNTRY != "Tanzania", bbox = tz,
         ext = 1.3, is.master = T) +
  tm_polygons(col = "grey90", colorNULL = NULL, border.col = "black") +
tm_shape(malaria_sp) +
  tm_symbols(col = "prev100", shape = 21, border.col = "black",
             palette = pal, size = .1, border.lwd = .7,
             style = "fixed", breaks = brks, legend.col.show = F) +
  ...
  tm_layout(design.mode = F, legend.bg.color = "white",  
            ...
            outer.margins = 0, asp = 0) 
# save the map
tmap_save(filename = "figs/figure1_prevmap_background.pdf", width = 6)  
